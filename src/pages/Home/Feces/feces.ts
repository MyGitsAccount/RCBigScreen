 ...
 * 获取粪污设计统计的echarts配置
 */
export function genEquipmentChartOption(list: TabList[]): any {
  // console.log(list);
  // const temp = [
  //   {
  //     name: '砌石坝',
  ...
  list.forEach((item, index) => {
    temp.push({
      ...
      number: item.value,
      rate: ((item.value / all) * 100).toFixed(0),
      itemStyle: {
        // 透明度
        opacity: 0.45,
        // 扇形颜色
  ...
  series.push({
    ...
    label: {
      ...
      formatter: (params: any) => {
        return params.data.name + '\n' + params.data.number + params.data.unit + ' ' + params.data.rate + '%';
 ...
 * @returns
 */
export function genUseRatioChartOption(params: any): object {
  const days: string[] = [];
  const rates: number[] = [];
  params.forEach((item: any) => {
    days.push(item.dname);
    rates.push(Number(item['value']));
  });
          ...
          normal: {
            color: {
              ...
              colorStops: [
                {
                  offset: 0,
                  color: 'rgba(23, 79, 104, 1)', // 0% 处的颜色
                },
                {
                  offset: 1,
                  color: 'rgba(23, 79, 104, 0.02)', // 100% 处的颜色
                },
              ],
